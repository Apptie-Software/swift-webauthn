...
public struct PublicKeyCredentialCreationOptions: Codable, Sendable {
    ...
    public init(from decoder: any Decoder) throws {
        ...
        let _challenge = try container.decode(URLEncodedBase64.self, forKey: .challenge)
        if let bytes = _challenge.decodedBytes {
        ...
        if let _timeout = try container.decodeIfPresent(Int64.self, forKey: .timeout) {
            self.timeout = Duration.milliseconds(_timeout)
        } else {
            self.timeout = nil
        }
    ...
    init(challenge: [UInt8], user: PublicKeyCredentialUserEntity, relyingParty: PublicKeyCredentialRelyingPartyEntity, publicKeyCredentialParameters: [PublicKeyCredentialParameters], timeout: Duration?, attestation: AttestationConveyancePreference) {
